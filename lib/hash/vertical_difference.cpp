...
nonstd::expected<uint64_t, std::string> vertical_difference(const Image& img)
{
  ...
  return vertical_difference(maybe_grey.value());
}
...
nonstd::expected<uint64_t, std::string> vertical_difference(const boost::gil::gray8_image_t& img)
{
  ...
  const auto maybe_small = Resize::transform(img, point_t(8, 9));
  ...
  for (unsigned int x = 0; x < 8; ++x)
  {
    ...
    {
      result += static_cast<uint64_t>(small_view(x, y) > small_view(x, y + 1)) << shift;
      ++shift;
    }
  }
  return result;
