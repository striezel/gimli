...
:: first image (reference) is undetermined
"%EXECUTABLE%" "%2"\file.undetermined "%2"\rgb.jpeg
if %ERRORLEVEL% NEQ 0 (
  echo Executable did not exit with code 0.
  exit /B 1
)

:: second image (comparison) is undetermined
"%EXECUTABLE%" "%2"\rgb.jpeg "%2"\file.undetermined
if %ERRORLEVEL% NEQ 0 (
  echo Executable did not exit with code 0.
  exit /B 1
)
